...
export default class CustomRouter {
  ...
  handlePolicies = (policies) => (req, res, next) => {
    ...
    jwt.verify(token, PRIVATE_KEY, (error, credential) => {
      if (error)
        return res
          .status(401)
          .send({ error: "Invalid Token, unauthorized!", erro: error });
      //Token OK
      ...
      if (!policies.includes(user.role.toUpperCase()))
        return res.status(401).send({
          error:
            "This user doesn't have the required privileges, verify your roles!",
        });
  ...
  generateCustomResponses = (req, res, next) => {
    ...
    res.sendClientError = (error) =>
      res.status(400).send({ status: "Client Error, bad request.", error });
    ...
    res.sendForbiddenError = (error) =>
      res.status(401).send({
        error:
          "Invalid Token or user with no access, Unauthorized please verify your roles!",
      });
    next();
  };
}
